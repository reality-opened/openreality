...
def _stub_torch(monkeypatch):
    """Inject a minimal `torch` stub so server modules can be imported without
    the real PyTorch (which is GPU-heavy and not installed in CI).

    Only the attributes that appear at module-level in streaming_slam.py / app.py
    are stubbed.  Method bodies that call torch are never executed by these tests
    because the conftest replaces StreamingSLAM / the SLAM stacks with fakes.
    """
    torch_mod = types.ModuleType("torch")
    torch_mod.cuda = types.SimpleNamespace(
        is_available=lambda: False,
        get_device_name=lambda n="": "",
    )
    torch_mod.hub = types.SimpleNamespace(get_dir=lambda: "/tmp")
    torch_mod.load = lambda *a, **kw: {}
    torch_mod.from_numpy = lambda x: x
    torch_mod.Tensor = type("Tensor", (), {})
    # Minimal nn stub (streaming_slam doesn't use torch.nn at top level but
    # other imports in the chain might reference it).
...
def load_streaming_slam_module(monkeypatch):
    ...
    # torch is imported at the top of streaming_slam.py; stub it so CI (no GPU)
    # can import the module.  Real torch behaviour is never reached because the
    # Solver / VGGT stubs below replace every heavy call site.
...
def load_app_module(monkeypatch):
    ...
    # torch is imported at the top of app.py; stub it for CI (no GPU / no PyTorch).
    _stub_torch(monkeypatch)
    ...
    class FakeFlask:
        def __init__(self, *args, **kwargs):
            self.routes = []
            # Blueprints app.py registers at module level (the demo one-liner,
            # docs/demo-2026-07). Recorded so tests can assert registration.
    ...
    # Route-recording Blueprint stand-in so `from server.oreos import oreos_bp`
    # (executed at server.app import time) works under the stubbed flask.
    class FakeBlueprint:
        def __init__(self, *args, **kwargs):
            self.routes = []
    ...
    # Records callbacks instead of running them; tests invoke them to exercise
    # after-response cleanup (e.g. the export zip temp-file removal).
    flask_mod.after_this_request_callbacks = []

    def _fake_after_this_request(fn):
        flask_mod.after_this_request_callbacks.append(fn)
        return fn

    flask_mod.after_this_request = _fake_after_this_request
